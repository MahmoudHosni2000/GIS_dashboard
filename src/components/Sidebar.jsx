...
const Sidebar = () => {
  ...
  const Menus = [
    ...
    { title: "التنوع البيولوجي", src: "biodiversity", gap: true, path: "/biodiversity" },
    { title: "الاستدامة والمرونة المناخية", src: "sustainability", path: "/sustainability" },
    ...
    <div className="flex">
      <div className={`${open ? "w-64" : "w-20"} bg-black h-screen p-4 pt-8 relative duration-300`}>
        <img
          ...
          className={`absolute cursor-pointer -left-3 top-9 w-7 border-dark-purple border-2 rounded-full ${!open ? "rotate-180" : ""}`}
        ...
        <div className="flex flex-row-reverse gap-x-4 items-center">
          ...
          />
          <h1 className={`text-white text-right text-lg font-bold duration-200 ${!open ? "scale-0" : ""}`}>
            مؤشرات نظام المعلومات الجغرافية
          </h1>
        </div>
        <ul className="pt-6">
